			...
			>
				...
				<div class="flex items-center justify-between border-b border-[#27272a] px-5 py-4">
					<div class="flex items-center gap-2">
						...
						<Dialog.Title class="m-0 text-base font-bold text-white"
							>{m.edit_subject()}</Dialog.Title
						>
					</div>
					<Dialog.Close
				...
				<div class="flex flex-col gap-4 p-5">
									...
									{#if draft.time}
										<span
											class="rounded-md border border-[#111111] px-1.5 py-0.5 font-[JetBrains_Mono,monospace] font-semibold {previewIsDarkBg
												...
												: 'bg-[#111111]/90 text-white'}">{draft.time}</span
										>
									{/if}
									{#if draft.room}
										<span
											class="rounded-md border border-[#111111] px-1.5 py-0.5 font-[JetBrains_Mono,monospace] font-semibold {previewIsDarkBg
												? 'bg-white/95 text-[#111111]'
												: 'bg-[#111111]/90 text-white'}">{draft.room}</span
										>
									{/if}
									{#if draft.section}
										<span
											class="rounded border border-[#111111] px-1.5 py-0.5 font-[JetBrains_Mono,monospace] font-bold {previewIsDarkBg
												...
												: 'bg-[#111111] text-white'}">Sec {draft.section}</span
										>
									{/if}
									{#if draft.type}
										<span
											class="rounded border border-[#111111] px-1.5 py-0.5 font-[JetBrains_Mono,monospace] font-bold uppercase {previewIsDarkBg
												...
												: 'bg-white text-[#111111]'}">{draft.type}</span
										>
									{/if}
								</div>
							</div>
					...
					<div class="grid grid-cols-2 gap-3">
						<div class="flex flex-col gap-1">
							<label class="text-xs font-medium text-[#a1a1aa]" for="blockTimeInput"
								>{m.time()}</label
							>
							<input
								id="blockTimeInput"
						...
						<div class="flex flex-col gap-1">
							<label class="text-xs font-medium text-[#a1a1aa]" for="blockRoomInput"
								>{m.room()}</label
							>
							<input
								id="blockRoomInput"
								...
								onChange={(newHex) => {
									if (draft) {
										const existing = palette.find(
											(p) => p.color.toLowerCase() === newHex.toLowerCase()
										);
										if (existing) {
