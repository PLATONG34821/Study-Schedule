...
<Popover.Root bind:open={isOpen}>
	...
	<Popover.Portal>
		<Popover.Content
			class="z-[999] flex flex-col items-center gap-3 rounded-2xl border border-[#3f3f46] bg-[#18181b] p-4 text-white shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95"
			sideOffset={8}
		>
			<div bind:this={containerEl} class="flex items-center justify-center"></div>

			<div class="flex w-full items-center justify-between gap-2 border-t border-[#27272a] pt-3">
				<span class="font-[JetBrains_Mono,monospace] text-xs font-bold text-[#a1a1aa] uppercase">{currentColor}</span>
				<button
					type="button"
